...
```
### 1.2、隐含操作导致空指针 比如Integer Float 和int float类型比较大小时的拆箱 Switch中传入的String内容等
```
...
```
### 1.7、List Set 等集合类的addAll方法，要确保传入的集合不为null
```
    ...
    public boolean addAll(Collection<? extends E> c) {
		//如果传的集合c为null 就会抛出空指针
        Object[] a = c.toArray();
        ...
    }

	//正确使用<==
	if (goods.getSnList() != null && goods.getSnList().size() > 0) {
        allSerailList.addAll(goods.getSnList());
    }
